...
function NewContract({ user }) {
  ...
  async function saveDraft() {
    ...
    try {
      ...
      const storageKey = `tidetracts_draft_${user.id}`;
      ...
      setSaveStatus("saved");
      setError("Couldn't save draft locally: " + err.message);
    }
  }
  ...
  async function restoreDraft() {
    ...
    try {
      ...
      draftId.current = draftData.id;
      setTitle(draftData.title);
      setSignerName(draftData.signer_name || "");
      setSelectedId(draftData.selected_id);
      setLastSaved(new Date(draftData.updated_at));
      
      // Note: we can't restore files from localStorage, but we preserve the metadata
      // User will need to re-upload PDFs but all other settings are restored
      if (draftData.documents && draftData.documents.length > 0) {
        setError(`Draft restored! Note: You'll need to re-upload your ${draftData.documents.length} PDF(s), but all your settings were saved.`);
      }
    } catch (err) {
      console.error("Couldn't restore draft:", err);
    }
  }
  ...
  // Auto-save effect
  useEffect(() => {
    if (!user || !title.trim() || documents.length === 0) return;
    ...
    // Set new timer for auto-save (30 seconds of inactivity)
  ...
  }, [documents, title, signerName, selectedId]);

  // Restore draft on mount
  useEffect(() => {
    if (user && documents.length === 0 && !title) {
      restoreDraft();
    }
  }, [user]);
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (saveStatus === "unsaved" && documents.length > 0 && title.trim()) {
        e.preventDefault();
        ...
        <div className="builder-grid">
          <aside className="builder-sidebar">
            ...
            <div className="save-controls">
              <button className="secondary save-button" onClick={saveDraft} disabled={busy || !title.trim() || documents.length === 0}>
                {saveStatus === "saving" ? "Saving..." : saveStatus === "saved" && lastSaved ? "✓ Saved" : "Save draft"}
              </button>
              <span className="save-status">
                {saveStatus === "unsaved" && documents.length > 0 && title.trim() ? "Unsaved changes" : lastSaved ? `Last saved ${Math.max(0, Math.floor((Date.now() - lastSaved) / 60000))}m ago` : ""}
              </span>
            </div>
  ...
  return (
    <div className="signature-pad-container">
      <div className="signature-size-controls">
        <label htmlFor="sig-size">Size:</label>
        <div className="size-buttons">
          {sizePresets.map((preset) => (
            <button
              key={preset.value}
              type="button"
              className={`size-btn ${height === preset.value ? 'active' : ''}`}
        ...
        />
        <span className="size-value">{height}px</span>
      </div>
      <div className="signature-pad" style={{ height: `${height}px` }}>
        <canvas ref={canvasRef} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} />
        <button type="button" className="clear-sign" onClick={clear}>Clear</button>
        <span>Sign here</span>
      </div>
    </div>
  );
